...
def calc_SF(fname,f_ratio_name):
    ...
    #If we only want to consider events with a certain filter
    ...
    #We don't need to normalize since we are just going to use the SF
    ...
    sf_unc_up = tot_unc_up/eff_rw
    sf_unc_down = tot_unc_down/eff_rw
    ...
    print(eff_str)
    f_ratio.Close()
    return sf_nom,sf_unc_up,sf_unc_down,eff_nom,eff_rw,tot_unc_up,tot_unc_down
...
for year in years:
    for process in processes:
        ...
        if year=="2016APV":
            ratio_file = "data/ratio_2016.root"
        sf_nom,sf_unc_up,sf_unc_down,eff_nom,eff_rw,tot_unc_up,tot_unc_down = calc_SF("merged.h5",ratio_file)
